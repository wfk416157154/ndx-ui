        ...
        <el-tabs type="border-card" v-model="activeTab" @tab-click="tabClick">
            <el-tab-pane :label="item.jcmc" :name="item.id+''" v-for="(item,index) in materaialList" :key="index">
                ...
                <div style="width: 50%;margin-top: 20px;float: left">
                    <el-tree :ref="'tree'+item.id" :data="kjTreeList" :props="defaultProps" :filter-node-method="filterNode" :highlight-current="true" :default-expand-all="true" @node-click="handleNodeClick"></el-tree>
                </div>
...
export default {
    ...
    methods: {
        ...
        handleNodeClick(data) {
            ...
            this.kcrwList = [];
            queryTeachingTemplateList({ parentId: data.id, isFilterNotNumber: true, isMyselfQuery: true }).then(res => {
