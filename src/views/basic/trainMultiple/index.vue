<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="选项名称" prop="xxmc">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['basic:trainMultiple:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['basic:trainMultiple:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['basic:trainMultiple:remove']"
        >删除</el-button>
      </el-col>
    <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['basic:trainMultiple:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table style="width: 100%;font-size : 18px" border v-loading="loading" :data="trainMultipleList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="选项名称" prop="xxmc">
          <el-input v-model="form.xxmc" placeholder="请输入选项名称" />
        </el-form-item>
        <el-form-item label="组件id" prop="zjid">
...
<script>
import { listTrainMultiple, getTrainMultiple, delTrainMultiple, addTrainMultiple, updateTrainMultiple } from "@/api/basic/trainMultiple";
  ...
  data() {
    return {
      ...
      upload: {
          ...
          // 上传的地址
          url: process.env.VUE_APP_BASE_API + "basic/trainMultiple/importData"
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      listTrainMultiple(this.queryParams).then(response => {
        this.trainMultipleList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加培训内容（多选）";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getTrainMultiple(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改培训内容（多选）";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTrainMultiple(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTrainMultiple(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除培训内容（多选）编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delTrainMultiple(ids);
        }).then(() => {
          this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('basic/trainMultiple/export', {
        ...this.queryParams
      }, `培训内容（多选）.xlsx`)
    },
      ...
      /** 导入按钮操作 */
      handleImport() {
          this.upload.title = "培训内容（多选）数据导入";
          this.upload.open = true;
      },
      /** 下载模板操作 */
      importTemplate() {
          this.download('basic/trainMultiple/importTemplate', {
              ...this.queryParams
          }, `培训内容（多选）-导入模板.xlsx`)
      },
      // 文件上传中处理
