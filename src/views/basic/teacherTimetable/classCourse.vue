    ...
    <div class="app-container">
        ...
        <el-form :model="queryParams" ref="queryForm" :rules="queryParamsRules" :inline="true" label-width="68px">
            <el-form-item label="校区名称" prop="xqid" v-hasPermi="['basic:school:list']" label-width="100px">
                <el-select v-model="queryParams.xqid" @change="getSchoolListId" filterable placeholder="请选择校区名称">
            ...
            <el-form-item label="日语班" label-width="100px">
                <el-select v-model="queryParams.bjid" filterable placeholder="请选择班级名称">
                    <el-option v-for="item in listBjclass" :key="item.id" :label="item.rybjmc" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <!-- <el-form-item label="年份" prop="kzzd2">
                <el-select v-model="queryParams.kzzd2" placeholder="请选择年份" clearable size="small">
                    <el-option v-for="dict in yearList" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictLabel" />
                </el-select>
            </el-form-item>
            <el-form-item label="课表类型" prop="kbType" label-width="100px">
                <el-select v-model="queryParams.kbType" placeholder="请选择课表类型" clearable size="small">
                    <el-option v-for="dict in kbTypeOptionsEL" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
                </el-select>
            </el-form-item> -->
            <el-form-item>
                <el-button type="primary" icon="el-icon-search" @click="switchingClasses(queryParams.bjid)">搜索</el-button>
            </el-form-item>
        </el-form>
        <div style="margin-bottom : 20px">
            <el-form :model="queryParams" :inline="true">
                <el-form-item label="老师姓名" label-width="100px">
                    ...
                    <el-button style="margin-left : 10px" type="primary" icon="el-icon-search" @click="lsQuery">查询老师</el-button>
                </el-form-item>
            </el-form>
        </div>
        <el-row :gutter="20">
            <el-col :span="6" :xs="24">
                <el-card>
                    <el-tabs type="card" v-model="activeTab" @tab-click="switchingClasses(activeTab)">
                        <el-tab-pane v-for="item in listBjclass" :key="item.id" :label="item.rybjmc" :name="item.id">
                            <div class="wrap-info">
                ...
                <el-card class="box-card">
                    ...
                    <div>
                        <el-button type="success" size="mini" @click="addKbDialogFormVisible = true">新增课表</el-button>
                        <el-dialog title="课表新增" :visible.sync="addKbDialogFormVisible">
                            <el-form :model="queryParams" ref="queryForm" :rules="queryParamsRules" :inline="true" label-width="68px">
                                <el-form-item label="年份">
                                    <el-select style="margin-right: 10px" v-model="queryParams.kzzd2" placeholder="请选择年份" clearable size="small">
                                        <el-option v-for="(item, index) in bjkbStartDate" :label="item" :value="item" :key="index" />
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="课表类型">
                                    <el-select style="margin-right: 10px" v-model="queryParams.kbType" placeholder="请选择课表类型" clearable size="small">
                                        <el-option v-for="dict in kbTypeOptionsEL" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
                                    </el-select>
                                </el-form-item>
                            </el-form>
                            <div slot="footer" class="dialog-footer">
                                <el-button @click="addKbDialogFormVisible = false">取 消</el-button>
                                <el-button type="primary" @click="handleClassCourseTemplateAdd">确 定</el-button>
                            </div>
                        </el-dialog>
                    </div>
                    <div v-for="(item, index) in classCourseBasicList" :key="index" class="list-kb-item" :class="[item.id === courseId ? 'courseClass' : '']">
                        <el-switch v-model="item.sfqy" @change="setSfqy(item)" active-color="#13ce66" inactive-color="#ff4949"></el-switch>
                        <div>
                            <el-link type="success" style="margin-right: 10px" @click="getCopyContent(item)">
                                复制课表
                            </el-link>
                            <el-dialog title="复制课表" :visible.sync="copyDialogFormVisible">
                                <el-form :model="copyKbForm">
                                    <el-form-item label="年度">
                                        <el-select style="margin-right: 10px" v-model="copyKbForm.updateAfterNd" placeholder="请选择年份" clearable size="small">
                                            <el-option v-for="(item, index) in bjkbStartDate" :label="item" :value="item" :key="index" />
                                        </el-select>
                                    </el-form-item>
                                    <el-form-item label="课表类型">
                                        <el-select style="margin-right: 10px" v-model="copyKbForm.updateAfterKbType" placeholder="请选择课表类型" clearable size="small">
                                            <el-option v-for="dict in kbTypeOptionsEL" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
                                        </el-select>
                                    </el-form-item>
                                </el-form>
                                <div slot="footer" class="dialog-footer">
                                    <el-button @click="copyDialogFormVisible = false">取 消</el-button>
                                    <el-button type="primary" @click="copySubmit(item)">确 定</el-button>
                                </div>
                            </el-dialog>
                        </div>
                        <div style="float: right" @click="getCourse(item)">
                            <el-link type="primary">{{ item.nd }} 年度- {{ item.kbTypeName }}-{{
            ...
            <el-col :span="18" :xs="24">
                <el-card>
                    ...
                    <el-button type="info" size="mini" @click="clickEdit">点击修改</el-button>
                    <el-button type="primary" plain icon="el-icon-document-copy" size="mini" v-if="false" @click="showCopyDialog">复制当前已启用的课表</el-button>
                    <div ref="prent">
                        <el-tabs v-model="tabsActiveTab">
                            <el-tab-pane label="班级课表" name="kb">
                                <div style="margin-bottom: 10px">
                                    <el-form :model="queryParams" ref="queryForm" :rules="queryParamsRules" :inline="true" label-width="68px">
                                        <el-button type="primary" icon="el-icon-plus" size="mini" @click="insertTimetable">新增课程</el-button>
                                        <el-button type="success" icon="el-icon-check" size="mini" :disabled="btnDisabled" @click="submitTimetable">保存课表</el-button>
                                ...
                                <el-table v-if="classCourseList.length > 0" ref="multipleTable" :data="classCourseList" tooltip-effect="dark" style="width: 100%; font-size: 18px" border @selection-change="handleSelectionChange" :cell-style="tableStyle" :header-cell-style="tableStyle" @cell-click="cellRow">
                                    <el-table-column type="selection" width="55"></el-table-column>
                                    <el-table-column label="开始时间" width="120">
                                        <template slot-scope="scope">
                                            <el-time-select style="width: 100%" placeholder="开始时间" v-model="scope.row.kssj" :picker-options="chooseTimeObj"></el-time-select>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="结束时间" width="120">
                                        <template slot-scope="scope">
                                            <el-time-select style="width: 100%" placeholder="结束时间" v-model="scope.row.jssj" @change="sjYZ(scope.row)" :picker-options="chooseTimeObj"></el-time-select>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="课程类型" width="120">
                                        <template slot-scope="scope">
                                            <el-select v-model="scope.row.kcType" placeholder="请选择">
                                                <el-option v-for="item in kcType" :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue"></el-option>
                                            </el-select>
                                        </template>
                                    </el-table-column>
                                    <el-table-column ref="demo" label="周一" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.mondayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.mondayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                    ...
                                    <el-table-column label="周二" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.tuesdayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.tuesdayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
                                    ...
                                    <el-table-column label="周三" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.wednesdayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.wednesdayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
                                    ...
                                    <el-table-column label="周四" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.thursdayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.thursdayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
                                    ...
                                    <el-table-column label="周五" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.fridayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.fridayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
                                    ...
                                    <el-table-column label="周六" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.saturdayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.saturdayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
                                    ...
                                    <el-table-column label="周日" width="120">
                                        <template slot-scope="scope">
                                            <div v-if="scope.row.sundayDetails.length > 0">
                                                <div @click="addCourse(scope.$index)" v-for="(item, index) in scope.row.sundayDetails" :key="index">
                                                    <el-link v-if="item.kzzd1 == 0" type="success">{{ item.ybj }} : 共 {{ item.yjskrs }} 人</el-link>
                                                    <el-link v-else type="success">全部</el-link>
                                                </div>
                                            </div>
        ...
        <el-dialog :title="copyTitle" :visible.sync="copyOpen" width="500px" append-to-body>
            ...
            <el-date-picker v-model="chooseNianfen" type="year" :disabled="true" placeholder="选择年">
            </el-date-picker>
            <el-select style="margin-right: 10px" v-model="chooseKblx" placeholder="请选择课表类型" clearable size="small">
                <el-option v-for="dict in kbTypeOptionsEL" :key="dict.dictValue" :label="dict.dictLabel" :value="dict.dictValue" />
            </el-select>
            <div slot="footer" class="dialog-footer">
