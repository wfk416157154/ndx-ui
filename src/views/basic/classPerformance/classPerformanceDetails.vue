<template>
  <div class="app-container">
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-else>
              ...
              <span
                v-else-if="scope.row[item.colour] == 3"
                style="
                  background: red;
                  display: inline-block;
                  width: 100%;
    ...
    <div v-else>
      <scoreChart
        ref="getChart"
        :getExaminationType="getExaminationType"
        :query="queryParams"
      />
    </div>
    <!--修改成绩对话框 -->
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["cjform"].validate((valid) => {
        if (valid) {
          if (this.cjform.id != null && this.cjform.kscj != null) {
            let rep = new RegExp(/[\w]+/gi);
            console.log(rep.test(this.cjform.kscj));
            if (rep.test(this.cjform.kscj)) {
              this.cjform.remark = this.cjform.kscj;
              this.cjform.kscj = null;
            } else {
              this.cjform.remark = null;
            }
