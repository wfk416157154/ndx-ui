        ...
        <el-form :inline="true" :model="queryParams" ref="teachingPlanForm">
            <el-form-item label="教材" prop="jcid">
                <el-select filterable v-model="queryParams.jcid" placeholder="请选择教材">
                    <el-option v-for="(item, index) in listTeachingMaterial" :key="index" :label="item.jcmc" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="开班时间" prop="sjall">
                <el-date-picker v-model="queryParams.sjall" value-format="yyyy-MM-dd" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
            </el-form-item>
            <el-form-item label="学期" prop="kbxq">
                <el-select filterable v-model="queryParams.kbxq" placeholder="请选择学期">
                    <el-option v-for="(item, index) in teachingTermConfig" :key="index" :label="item.dictLabel" :value="item.dictValue"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="届数" prop="rybjmc">
                <el-input v-model="queryParams.rybjmc" placeholder="请输入届数"></el-input>
            </el-form-item>
            ...
            <el-form-item label="班级" prop="rybjid">
                <el-select filterable v-model="queryParams.rybjid" placeholder="请选择班级">
                    <el-option :label="item.rybjmc" :value="item.id" v-for="(item, index) in classList" :key="index"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="老师" prop="lsid">
                <el-select filterable v-model="queryParams.lsid" placeholder="请选择老师">
                    <el-option v-for="(item, index) in teacherList" :key="index" :label="item.lsxm" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="教学进度" prop="jxjd">
                <el-select filterable v-model="queryParams.jxjd" placeholder="请选择">
                    <el-option v-for="(item, index) in teachingProgress" :key="index" :label="item.dictLabel" :value="item.dictValue"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="课程类型" prop="kclx">
                <el-select filterable v-model="queryParams.kclx" placeholder="请选择教学内容">
                    <el-option v-for="(item, index) in kcType" :key="index" :label="item.dictLabel" :value="item.dictValue"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
            ...
            <ul>
                ...
                <li style="width: 600px">
                  ...
                  "></span>
                    ...
                    margin-right: 5px;
                  "></span>
                                <span style="isplay: inline-block; margin-right: 10px">跳过时间</span>
                            </li>
                        </ul>
                    </div>
              ...
              <li></li>
                ...
                <li>
                    <div style="display: flex; align-items: center">
                        ...
                        <div>
                            <span style="cursor: pointer" :class="['MonthView' == ifContent ? 'active-color' : '']" @click="toComponent('MonthView', 1)">月</span>
                            <el-divider direction="vertical"></el-divider>
                            <span style="cursor: pointer" :class="['SemesterView' == ifContent ? 'active-color' : '']" @click="toComponent('SemesterView', 2)">学期</span>
                            <el-divider direction="vertical"></el-divider>
                            <span style="cursor: pointer" :class="['WholeView' == ifContent ? 'active-color' : '']" @click="toComponent('WholeView', 3)">全部</span>
                        </div>
                    </div>
                </li>
            </ul>
                    ...
                    <div class="content-top">
                        <div class="content-top-left">
                            <div class="info-left">
                                ...
                                <div>
                                    <span>正常教学计划 :</span>
                                    <span v-if=" item.normalClassPlanObj  && item.normalClassPlanObj.qbsw != '1'">{{
                                ...
                                <div>
                                    <span>时间区间 :</span>
                                    <span>{{ parseTime(item.kbsj, "{y}-{m}-{d}") }}至{{
                      parseTime(item.gksj, "{y}-{m}-{d}")
                    }}</span>
                                </div>
                            </div>
                            <div class="info-right">
                                <el-button size="mini" style="width: 80px" @click="toAddTeaching('addTeaching', item)">修正|查看</el-button>
                                <br />
                                <el-button size="mini" style="width: 80px">调用</el-button>
                                <br />
                                <el-button size="mini" style="width: 80px" @click="handleExport(item.rybjid)">导出</el-button>
                            </div>
                        </div>
                    ...
                    <el-divider>{{ termDict.dictLabel }}</el-divider>
                        ...
                        <div>
                            <div v-if="item.lessonProcessObjList.length > 0">
                                <ndx-progress :progressItem="item.lessonProcessObjList" :propressTitle="item.classPlanNodeInfoList" :nowCourseName="item.nowCourseName" />
                            </div>
                            <div v-if="
                  item.reviewProcessObjList &&
                  item.reviewProcessObjList.length > 0
                ">
                                <el-button type="success" @click="getReviewProcessList(item.reviewProcessObjList)">查看复习进度</el-button>
                                <el-dialog title="查看复习进度" :visible.sync="dialogVisxibleFx" width="80%">
                                    <div v-for="(row, j) in reviewProcessList" :key="j">
                                        <p style="margin-right: 30px; width: 100%">
                                            {{ row.reviewName }}
                                        </p>
                                        <el-progress :text-inside="true" :stroke-width="22" :percentage="row.reviewProcessList[0].percentage" status="warning"></el-progress>
                                    </div>
                                    <span slot="footer" class="dialog-footer">
                                        <el-button @click="dialogVisxibleFx = false">取 消</el-button>
                                    </span>
                                </el-dialog>
                            </div>
                        </div>
