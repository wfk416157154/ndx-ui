<template>
    <div class="train-learning">
        <div class="top-video">
            ...
            <div class="video">
                <ndx-video ref="video" @realTimePush="realTimePush" :executeDocument="executeDocument" @executionFun="testQuestions"></ndx-video>
            </div>
            <div class="jd">
        ...
        <el-dialog title="测试题" :close-on-click-modal="false" :show-close="false" :close-on-press-escape="false" :visible.sync="dialogFormVisible">
                ...
                <ul>
                    <li>
                        <div v-for="(item, index) in videoQuestionData" :key="index">
                            ...
                            <el-radio v-model="item[index]" @change="toAnswer(item[index], item.answer, index)" label="A">A, {{ item.optionA }}</el-radio>
                            <el-radio v-model="item[index]" @change="toAnswer(item[index], item.answer, index)" label="B">B, {{ item.optionB }}</el-radio>
                            <el-radio v-model="item[index]" @change="toAnswer(item[index], item.answer, index)" label="C">C, {{ item.optionC }}</el-radio>
                            <el-radio v-model="item[index]" @change="toAnswer(item[index], item.answer, index)" label="D">D, {{ item.optionD }}</el-radio>
                        </div>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    created() {
        ...
        selectFileList({ kzzd1: this.studyList.videoFileId }).then(async (res) => {
            ...
            this.executeDocument.videoUrl = `${process.env.VUE_APP_BASE_API
                }/file/play?fileId=${res.rows[0].id}&Authorization=${"Bearer " + getToken()
                }`;
        });
        learnRecordList({
        ...
        testQuestions(data) {
            videoQuestionList({ trainVideoId: this.studyList.spid }).then((res) => {
                ...
                if (this.videoQuestionData.length == 0) {
                    this.msgSuccess("视频学习已完成，请及时上传笔记");
