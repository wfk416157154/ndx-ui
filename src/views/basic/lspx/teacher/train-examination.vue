    ...
    <div class="train-examination" ref="examination">
        <div class="content">
                    ...
                    <ul>
                        <li v-for="(item,index) in getTrainPaperTopicListTem.danxuan" :key="index">
                            <div style="display : flex ;justify-content: space-between;">
                                <h4 style="width : 95%">{{++index}} ) {{item.tmbt}}</h4>
                                <h4>{{item.tmfz}} 分</h4>
                            </div>
                            <div class="answer" @click="danxuanFun(list,index,item)" v-for="(list,j) in item.optionList" :key="j">
                ...
                <div>
                    <ul>
                        <li v-for="(item,index) in getTrainPaperTopicListTem.panduan" :key="index">
                            ...
                            <div class="answer" @click="panduanFun(list,index,item)" v-for="(list,j) in item.optionList" :key="j">
                ...
                <div>
                    <ul>
                        <li v-for="(item,index) in getTrainPaperTopicListTem.tiankong" :key="index">
                            ...
                            <div>
                                <ul>
                                    <li style="margin : 20px 0px;align-items : center" v-for="(list,j) in item.optionList" :key="j">
                                        <div>
                                            <el-form>
                                                <el-form-item :label="list.kzzd1" label-width="70px">
                                                    <el-input @change="tiankongFun(list,index,j,item)" v-model="list['content']"></el-input>
                                                </el-form-item>
                                            </el-form>
                                        </div>
                                    </li>
            ...
            <div class="questions-answers" v-if="getTrainPaperTopicListTem.wenda.length > 0">
                ...
                <div>
                    <ul>
                        <li v-for="(list,index) in getTrainPaperTopicListTem.wenda" :key="index">
                            <div style="display : flex ;justify-content: space-between;">
                                <h4 style="width : 95%">{{++index}} ) {{list.tmbt}}</h4>
                                <div>
                                    <h4>{{list.tmfz}} 分</h4>
                                </div>
                            </div>
                            <div class="answer">
                                <h4>答题区</h4>
                                <editor v-model="list.kzzd5" :min-height="300" />
                            </div>
                        </li>
                    </ul>
                </div>
...
export default {
    ...
    created() {
        ...
        topicTypeList({ sjid: this.studyList.paperId }).then(res => {
