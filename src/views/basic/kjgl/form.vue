            ...
            <el-tab-pane label="上传修改后的课件" name="2">
                <div>
                    <div slot="footer" class="dialog-footer" style="margin: 0px 0px 10px 10px;">
                        <el-button
                            type="primary"
                            @click="updateCoursewareByStatus"
                            :disabled="!isUploadXgkj"
                        >启用修改后的课件</el-button>
...
} from "@/api/basic/operatingRecord";
...
export default {
    ...
    mounted() {
        ...
        if (this.parentParams.courseware) {
            /* 是否显示上传组件 */
            this.showUpload = this.parentParams.courseware.xgkjUrl
                ? false
                : true;
            /* 启用按钮 是否启用 */
            this.enableBtn =
                this.parentParams.courseware.status &&
                this.parentParams.courseware.status != "3"
                    ? false
                    : true;
            /* 显示原版课件 */
