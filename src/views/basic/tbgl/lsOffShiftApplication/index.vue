<template>
  <div style="width : 100%;height : 100%;padding : 10px; box-sizing : border-box;text-align : center">
    <h3>{{parentRowData.xsxm}}-退班申请</h3>
    <div>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <table style="width : 100%;" border="1" cellspacing="0">
          ...
          <tr>
            <td>入班时间</td>
            <td>{{parentRowData.rbsj}}</td>
          </tr>
          <tr>
            <td>退班规则</td>
            <td class="clearfix">
                <el-form-item label="退班类型" prop="tblx" align="left" >
                  <el-radio v-model="form.tblx"
                ...
                <el-form-item label="退班原因" prop="tbyy">
                  <el-input type="textarea" :rows="4" style="width: 100%" placeholder="请输入退班原因" v-model="form.tbyy"></el-input>
                </el-form-item>
            </td>
          </tr>
          <tr>
            ...
            <td class="clearfix" align="left">
                <el-form-item label="请选择" prop="yyxzlxyZt">
                  <el-radio v-model="form.yyxzlxyZt"
          ...
          <tr v-if="isShow">
            ...
            <td>
              <el-form-item label="(如需退费则输入)" prop="yhkh">
                <el-input type="textarea" :rows="3" placeholder="请输入银行卡号" v-model="form.yhkh"></el-input>
              </el-form-item>
            </td>
          </tr>
          <tr>
            <td>退班申请资料</td>
            <td>
              <div style="width : 180px;height : 270px; margin-top : 10px;margin-left: 80px">
                <el-upload
                  ref="upload"
                  :limit="uploadMaxNum"
                  accept=".*"
                ...
                >
                  ...
                  <div class="el-upload__text">
                    将文件拖到此处，或
                    <em>点击上传</em>
                    <div class="el-upload__tip" style="color:red;" slot="tip">提示：上传的文件大小不能超过50M！</div>
                  </div>
                </el-upload>
          ...
          <tr>
            <td>操作</td>
            <td>
              <el-button icon="el-icon-close" @click="$router.go(-1)">返回上个页面</el-button>
              <el-button type="primary" icon="el-icon-check" v-prevent-re-click @click="submitForm">提交申请</el-button>
            </td>
          </tr>
          </tbody>
        </table>
...
<script>
  ...
  import { addStuTuiban} from "@/api/basic/stuTuiban";
  export default {
    data() {
      return {
        upload: {
    ...
    created() {
      // 父页面跳转过来所带的行对象
      this.parentRowData = JSON.parse(this.$route.query.query);
      this.setFormData()
      // 教与学质量协议-状态码表
    ...
    methods: {
      setFormData(){
        ...
        this.form.rbsj=this.parentRowData.rbsj
        this.form.tbsj=new Date();
        this.form.shzt="1"// 申请
      },
      // 图片预览 大图
      ...
      jyxzlxyUploadSuccess(response, file, fileList) {
        this.form.yyxzlxyTpid=this.getFileAddResp(response, file, fileList,this.form.yyxzlxyTpid,"个图片")
      },
      // 收据-图片上传成功回调
      sjUploadSuccess(response, file, fileList) {
        this.form.sjTpid=this.getFileAddResp(response, file, fileList,this.form.sjTpid,"个图片")
      },
      // 退班申请资料-上传成功回调
      tbsqzlUploadSuccess(response, file, fileList) {
        this.form.tbsqzlTpid=this.getFileAddResp(response, file, fileList,this.form.tbsqzlTpid,"个文件")
      },
      // 获取上传成功后的回调
