<template>
  <div style="width : 100%; height : 100%; padding : 40px; box-sizing : border-box">
    <el-form ref="suspensionListForm" :model="suspensionListForm" :inline="true" label-width="80px">
      <el-form-item label="学校" prop="xqid">
        <el-select
          v-model="suspensionListForm.xqid"
          filterable
          placeholder="请选择校区"
          @change="xqmcOnChange"
        >
      ...
      <el-form-item label="班级" prop="rybjid">
        <el-select v-model="suspensionListForm.rybjid" placeholder="请选择班级">
          <el-option
            :label="item.rybjmc"
            :value="item.id"
      ...
      <el-form-item label="学生" prop="xsxm">
        <el-input v-model="suspensionListForm.xsxm" placeholder="请输入内容"></el-input>
      </el-form-item>
      <el-form-item label="休学状态" prop="shzt">
        <el-select v-model="suspensionListForm.shzt" placeholder="请选择休学状态">
          <el-option
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-refresh" @click="resetForm('suspensionListForm')">重置</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="offShiftListList" border style="width: 100%">
      ...
      <el-table-column prop="shzt" label="状态">
        <template slot-scope="scope">
          <div>
            <dict-tag :options="xxStatusList" :value="scope.row.shzt" />
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="kzzd1" label="驳回原因" width="300px"></el-table-column>
      <el-table-column label="操作" width="260px">
        <template slot-scope="scope">
          <el-button
            size="mini"
            v-if="scope.row.shzt == 2"
            @click="toPageitem('xxxqPage', scope.row)"
          >查看</el-button>
          <el-button
            size="mini"
            v-if="scope.row.shzt == 3"
            type="danger"
            @click="toPageitem('xxsqPage',scope.row)"
          >修改</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <pagination
      v-show="total>0"
      :total="total"
      :page.sync="suspensionListForm.pageNum"
      :limit.sync="suspensionListForm.pageSize"
      @pagination="getList"
    />
  </div>
</template>
...
import { listXiuxue } from "@/api/basic/xiuxue.js";
export default {
  data() {
    return {
      suspensionListForm: {
  ...
  methods: {
    ...
    // 数据
    getList() {
      listXiuxue(this.suspensionListForm).then(res => {
