<template>
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
            <el-form-item>
                <el-select v-model="queryParams.shzt" @change="getList" placeholder="请选择" clearable size="small">
                    <el-option v-for="(dict,index) in preparelesoonsStatus" :key="index" :label="dict.dictLabel" :value="dict.dictValue" />
                </el-select>
            </el-form-item>
        </el-form>
        ...
        <el-table style="width: 100%;font-size : 18px" v-loading="loading" :height="$root.tableHeight" border :data="lessonPreparationDataList">
            ...
            <el-table-column label="教案状态" align="center" :formatter="getPreparelesoonsStatus" prop="shzt" />
            <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                    <el-button type="success" size="mini" @click="viewRowData(scope.row)">查看</el-button>
                    <el-button type="danger" size="mini" v-if="['1','4'].includes(scope.row.shzt)" @click="editRowData(scope.row)">修改</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="查看备课信息" :visible.sync="viewDialogFormVisible">
            <div style="width : 100%;height : 100%;padding : 40px; box-sizing : border-box;text-align : center">
                <div>
                  ...
                  <editor v-model="form.kcapjxck" :disabled="true" :min-height="192" />
                            ...
                            <tr>
                                ...
                                <td style="text-align : left">
                                    <h3>已上传的图片</h3>
                                    <div class="clearfix">
                                        <div v-for="(item,index) in form.bkTpArr" :key="index" style="float : left;margin-right : 10px">
                    ...
                    <table style="width : 100%;" border="1" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align : left">
                                    <p>图片上传</p>
                                    <el-upload :action="upload.imgUrl" :headers="upload.headers" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-remove="handleRemove" :on-success="bkSuccess" :file-list="getImages">
                                    ...
                                    <p>文件上传</p>
                                    <el-upload ref="upload" :limit="1" accept="*" :headers="upload.headers" :action="upload.imgUrl" :disabled="upload.isUploading" :before-upload="beforeAvatarUploadZIP" :on-success="addFileSuccess" :on-remove="handleRemoveFiles" :auto-upload="false" :file-list="getFiles" drag>
                                        <i class="el-icon-upload"></i>
                                        <div class="el-upload__text">
                                            将文件拖到此处，或
                                            <em>点击上传</em>
                                        </div>
                                        <div class="el-upload__tip" style="color:red" slot="tip">提示：仅允许导入"{{upload.type}}"格式文件！</div>
                                    </el-upload>
                                    <br />
                                    <div slot="footer" class="dialog-footer">
