    ...
    methods: {
            ...
            if (this.$route.params.id && this.$route.params.id != ":id") {
                ...
                workLogListQuery({ id: this.rzid }).then((res) => {
                    if (res.data.length != 0) {
                        ...
                        this.ruleForm.basicTeacherWorkLogLessonList.map(
                            (value, index) => {
                                for (let i = 0; i < this.kcType.length; i++) {
                                    if (
                                        this.kcType[i].dictValue ==
                                        value.courseType
                                    ) {
                                        value.courseTypeName =
                                            this.kcType[i].dictLabel;
                        ...
                        this.ifExamination =
                            this.ruleForm.isExam == "1" ? true : false;
                        // 教室卫生照片回显
                        this.selectPhotoList(
                            (this.ruleForm.jswsFile =
                                this.ruleForm.jswsFile || secretKey()),
                            "files1"
                        );
                        // 学生表现照片回显
                        this.selectPhotoList(
                            (this.ruleForm.xsbxFile =
                                this.ruleForm.xsbxFile || secretKey()),
                            "files2"
                        );
                    }
                ...
                if (res.data.length != 0) {
                    ...
                    this.ruleForm.basicTeacherWorkLogLessonList.map(
                        (value, index) => {
                            for (let i = 0; i < this.kcType.length; i++) {
                                if (
                                    this.kcType[i].dictValue == value.courseType
                                ) {
                                    value.courseTypeName =
                                        this.kcType[i].dictLabel;
                                    this.ruleForm.basicTeacherWorkLogLessonList[
        ...
        handleImport() {
            ...
            for (let j = 0; j < this.getListExaminationPaper.length; j++) {
                if (
                    this.getListExaminationPaper[j].ksfw == this.ruleForm.kzzd4
                ) {
