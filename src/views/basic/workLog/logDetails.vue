  ...
  <div class="logDetails">
    <table style="width: 100%" border="1" cellspacing="0">
      <thead>
        <tr>
          <th></th>
          <th style="padding: 20px; box-sizing: border-box">
            {{ logItem.lsxm }} - {{ logItem.bjmc }} - {{ logItem.date }} -
            {{ logItem.status }}
          </th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          <td style="padding: 20px; box-sizing: border-box">
            <ul>
              <li
                v-for="(item, index) in logItem.basicTeacherWorkLogLessonList"
                :key="index"
              >
                <span style="line-height: 30px"
                  >{{ item.courseTypeName }} :</span
                >
        ...
        <tr>
          ...
          <td style="padding: 20px; box-sizing: border-box">
            ...
            <div>
              <div v-if="logItem.reason">
                <el-input
                  v-model="logItem.reason"
                  placeholder="未上传原因"
                  readonly
                ></el-input>
              </div>
              <el-radio
                v-model="logItem.ishg"
                label="1"
                @change="jianYan(true, logItem)"
                >合格</el-radio
              >
              <el-radio
                v-model="logItem.ishg"
                label="0"
                @change="jianYan(false, logItem)"
                >不合格</el-radio
              >
            </div>
          </td>
